...
import requests
from . import config
from .flow import search_fetch_and_analysis
...
# 共同討論
class AskReturnAndReward(Action):
    def name(self) -> Text:
        return "ask_return_and_reward"

    def run(self, dispatcher, tracker, domain) -> List[Dict[Text, Any]]:
        # get replier_id, room_id
        ...
        reply = "Would you like to record this question？（Only asker can respond）"
        dispatcher.utter_message(text=reply)
        return []


# 錯誤訊息解答
class ErrorMessageSearch(Action):
    def name(self) -> Text:
    ...
    def run(self, dispatcher, tracker, domain) -> List[Dict[Text, Any]]:
        ...
        # rank blocks
        rank, stack_items = search_fetch_and_analysis(qkey)

        result_title = [i['question']['title'] for i in stack_items]

        reply = "Thanks for waiting! Here are the results: "
        for i in range(len(result_title)):
            reply += ("<br>" + str(i + 1) + ".<a href=\"" + result_title[i] + "\">" + result_title[i] + "</a>")
        reply += ("\nClick the link to get more information\n\n"
                  "Hope this is helpful! You can access more functions in the menu.")
...
# 共同討論：是否匿名
class PopoverReturnIncognito(Action):
    def name(self) -> Text:
...
# 共同討論：討論標籤
class ReceivedDiscussTags(Action):
    def name(self) -> Text:
...
class AnalyzeAndSearch(Action):
    ...
    def run(self, dispatcher, tracker, domain) -> List[Dict[Text, Any]]:
        ...
        if "error message" in function.lower():
            ...
            ranks, stack_items = search_fetch_and_analysis(qkey)
            ...
            # Recommend more tags for the user
            more_keywords = []
...
class SelectKeyword(Action):
    ...
    def run(self, dispatcher, tracker, domain) -> List[Dict[Text, Any]]:
        ...
        for i in qkey:
            ...
            id += 1
        reply += ('</div><br>'
                  '<input id="addBtn" class="btn btn-primary purpleBtnInChatroom value="新增" onclick="wantAddKeyWord()">'
                  '<input id="doneBtn"class="btn btn-primary purpleBtnInChatroom" value="完成" onclick="doneKeyWord()">')
